...
const ITree = forwardRef((props, ref) => {
  ...
  const loop = data => {
    return data.map(item => {
      ...
      let tmpDisabled = item?.disabled||false;
      if(props?.disabledKeys?.length){
        tmpDisabled = props.disabledKeys.includes(item[key]);
      }
      if(props?.type&&props.type=='workRule'){//事项规则
        if(item.nodeType == 'ctlg'&&(!item.children||!item.children.length)){
          tmpDisabled=true;
        }
      }
  ...
  return (
    <div style={props.style} className={styles.main}>
      {props.isSearch ? (
        <div className={styles.main_header}>
          <Search
            style={{ width: '205px' }}
            onSearch={props.onSearch ? props.onSearch : onSearch}
