...
function Login({dispatch,user,closable}){
    const {modalVisible} = user
    ...
    // useEffect(()=>{
    //     setVisible(window.localStorage.getItem('visible')==='true'?true:false);
    // },[window.localStorage.getItem('visible')])
    // useEffect(()=>{
    //     setVisible(modalVisible);
    // },[modalVisible])
  const onFinish=(values)=>{
    if(values.oldPassword!=Base64.decode(window.localStorage.getItem('pkeys')?.slice(11))){
  ...
  const validatorPwd = (rule, value) => {
    ...
    const pwstrongRegularMemo = window.localStorage.getItem('pwstrongRegularMemo');
    if(value){
