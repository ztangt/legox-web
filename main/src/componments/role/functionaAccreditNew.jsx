...
function FunctionAccredit({ dispatch, role, layoutG }) {
    ...
    console.log(sysRegisteTree.list, sysModulesTree.list,'92929292')
    useEffect(() => {
        //获取角色注册系统
        dispatch({
    ...
    const treeDataHandle=(curSysModulesTree)=>{
      ...
      //获取当前注册系统的名称
      let curPaltForm = sysRegisteTree && sysRegisteTree.list?sysRegisteTree.list.filter(item=>item.id==registerId):'';
      let treeData = [{
        key:registerId,
    ...
    const loopChildren = (tree,sencodItem,preItem,newData)=>{
      ...
      tree.map((item,index)=>{
        ...
        if(item.children&&item.children.length&&item.children[0].nodeType!='BUTTON'){
          ...
          let buttons = [];
          if (item.children) {
            buttons = item.children.map((buttonItem)=>{
              return {
    ...
    const getChildrenTable=(selectedKeys,{selected,selectedNodes,node})=>{
      console.log(node, 'nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn')
      let newData = [];
            ...
            <ReSizeLeftRightCss
                leftChildren={
                    ...
                    <Tree
                        ...
                        showLine={true}
                        showIcon={false}
                        treeData={sysModulesTree && sysModulesTree.list?treeDataHandle(sysModulesTree.list):[]}
