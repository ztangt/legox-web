...
const Model = {
  ...
  effects: {
    ...
    *getSysRoles({ payload,callback }, { call, put,select }) {
      let namespace = payload.namespace;
      delete(payload.namespace);
      const {data} = yield call(apis.getSysRoles, payload,'','userAccredit');
      if(data.code==200){
        callback&&callback(data.data.list)
      }else if (data.code != 401 && data.code != 419 && data.code !=403) {
        message.error(data.msg);
    ...
    *getSelectedDatas({ payload,callback }, { call, put,select }) {
      ...
      const {data} = yield call(apis[curAction[0].action], newPayload,'','userAccredit');
      if(data.code==200){
    ...
    *getPosts({ payload,callback }, { call, put, select }) {
      ...
      try {
        ...
        console.log("data==00",data,"api=payload",payload)
        if(data.code==200){
