...
let parentNameAll = [];
function addForm ({dispatch,loading,onCancel,identityPosts,expandedPostKeys,identityObj,currentUg,identityModal,currentNode,checkedKeys}){
    console.log('identityPosts',identityPosts)
    console.log('identityObj',identityObj)
    ...
    function onExpand(expandedKeys, {expanded, node}){
      if(expanded){
        expandedPostKeys.push(node.key)
        dispatch({
            type:"userInfoManagement/updateStates",
            payload:{
              expandedPostKeys:simpleUnique(expandedPostKeys)
            }
        })
        if(!node.children){
          identityParent =[]
        ...
        let arr = [];
        arr.push(node)
        loop(arr,expandedPostKeys)
      }
    }
    function loop(arr,expandedPostKeys){
      arr.forEach(function(item,i) {
        expandedPostKeys.forEach(function(policy,j) {
          if(policy == item.key){
            expandedPostKeys.splice(j, 1)
          }
        });
        if(item.children&&item.children.length!=0){
          loop(item.children,expandedPostKeys)
        }
      });
      dispatch({
        type:"userInfoManagement/updateStates",
    ...
    function submitClick(){
      let arr = [];
      if(currentNode.nodeType == 'DEPT'){
        getParentKey(currentNode.nodeId,identityPosts)
      }
      let orgIds = currentNode.nodeType == 'ORG' ? currentNode.nodeId : orgObj.nodeId;
      let deptIds = currentNode.nodeType == 'DEPT' ? currentNode.nodeId : '';
      ...
      dispatch({
        type:"userInfoManagement/addIdentity",
        payload:{
        ...
        //   dispatch({
        //     type:"userInfoManagement/getUser",
        //     payload:{
...
export default (connect(({userInfoManagement,layoutG,loading})=>({
    ...userInfoManagement,
    ...layoutG,
    loading
  }))(addForm));
