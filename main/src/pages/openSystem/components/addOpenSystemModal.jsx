...
function addOpenSystemModal({ dispatch, openSystem }) {
    const [form] = Form.useForm()
    const { TextArea } = Input
    const layout = { labelCol: { span:  5.5}, wrapperCol: { span: 18 } }
    const { detailData } = openSystem
    ...
    const onFinish = (values) => {
        if (detailData.id) {
            dispatch({
                type: 'openSystem/updateOpenSystem',
                payload: {
                    id: detailData.id,
...
export default connect(({ openSystem }) => ({ openSystem }))(addOpenSystemModal)
