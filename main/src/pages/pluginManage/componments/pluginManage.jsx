...
import { dataFormat, getButton,getButtonByUrl } from '../../../util/util';
...
function PluginManage({
  ...
  console.log(menusUrlList,'menusUrlList');
  var viewDetailsModalRef; //查看Modalref
  useEffect(()=>{
    dispatch({
      type: 'pluginManage/getPlugTypeList',
      payload: {},
      callback: (data, plugTypeId) => {
        dispatch({
          type: 'pluginManage/getPlugList',
    ...
    columns: [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <div title={text}>
              {getButtonByUrl(menusUrlList,'view','','/pluginManage')? (
                <a
        ...
        render: (text, record, index) => {
          return (
            <div className={styles.actions}>
              {getButtonByUrl(menusUrlList,'update','','/pluginManage')&&
               (
                <a
                  style={{ marginLeft: '5px' }}
                  onClick={() => {
              ...
              {getButtonByUrl(menusUrlList,'delete','','/pluginManage')&&(
                <a
                  style={{ marginLeft: '5px' }}
                  onClick={() => {
              ...
              {getButtonByUrl(menusUrlList,'upMove','','/pluginManage')&&!(index == 0 && currentPage == 1) && (
                <a
                  style={{ marginLeft: '5px' }}
                  onClick={() => {
                    movePlugin(record, 1);
              ...
              {getButtonByUrl(menusUrlList,'downMove','','/pluginManage')&&
  ...
  const downLoadFile = ( record) => {
    dispatch({
      type: 'pluginManage/getDownFileUrl',
      payload: {
  ...
  function deletePlugin(plugId) {
    ...
    Modal.confirm({
      ...
      getContainer: () => {
        return document.getElementById('pluginManage_container');
      },
    });
  }
  ...
  function onDeletePlugType(node) {
    ...
    Modal.confirm({
      ...
      onOk() {
        dispatch({
          type: 'pluginManage/removePlugType',
          payload: {
  ...
  const leftRender = () => {
    return (
      <div className={styles.tree_left_header}>
        {getButtonByUrl(menusUrlList,'add','','/pluginManage')&& (
          <Button
            type="primary"
      ...
      <div className={styles.table_warp}>
        <div className={styles.header}>
          ...
          <div className={styles.right}>
            ...
            {getButtonByUrl(menusUrlList,'enable','','/pluginManage')&&(
              <Button
                onClick={() => {
                  enablePlugin(1);
                }}
            ...
            { getButtonByUrl(menusUrlList,'disable','','/pluginManage')&&(
              <Button
                onClick={() => {
                  enablePlugin(0);
                }}
...
export default connect(({ pluginManage, layoutG, loading, user }) => ({
  ...pluginManage,
  ...layoutG,
  loading,
  user,
