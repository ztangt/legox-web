...
function manageOperationLog({ dispatch, manageOperationLog }) {
  const {
    tableData,
    returnCount,
    limit,
  ...
  useEffect(() => {
    if(limit>0){
      getPubSafeBusinessOpera(1, limit, '');
    }
  }, [limit]);
  //接口列表
  const getPubSafeBusinessOpera = (start, limit, searchWord) => {
    dispatch({
      type: 'manageOperationLog/getPubSafeBusinessOpera',
      payload: {
        start,
        limit,
        searchWord,
  ...
  const onSearch = value => {
    getPubSafeBusinessOpera(currentPage, limit, value);
  };
  //搜索
  ...
  const changeInput = e => {
    dispatch({
      type: 'manageOperationLog/updateStates',
      payload: {
...
export default connect(({ manageOperationLog }) => ({ manageOperationLog }))(
  manageOperationLog,
);
