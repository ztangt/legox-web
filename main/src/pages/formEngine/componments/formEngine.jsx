...
function FormEngine({
  ...
  console.log(currentNode.key, 'currentNode======');
  ...
  const onPrint = () => {
    ...
    let path = `/support/formEngine/templateEditor?formId=${selectedFroms[0].formId}&version=${selectedFroms[0].mainVersion}`;
    window.open(`#${path}`, '_blank');
  };
  ...
  const saveBelongOrg = () => {
    ...
    if (selectedFroms.length > 0) {
      ...
      if (selectedFroms.length === 1) {
        // 查询回显数据
        dispatch({
          type: 'formEngine/queryBelongOrg',
          payload: {
            dataId: dataIds,
  ...
  const isShowFn = (record, code) => {
    var isShow = false;
    if (exsitList?.length < 3) {
      isShow = code == 'view' ? getButton(menus, 'view') : true;
    } else if (exsitList?.length == 3) {
      //包含删除按钮且删除按钮展示
      if (record.isDeploy != 1) {
        isShow = false; //不展示按钮
      } else {
      ...
      var notShowDelete = record.isDeploy == 1?true: exsitList?.toString()?.includes('delete')?false:true
      if ((record.isDeploy == 1||!exsitList?.toString()?.includes('delete'))&&code == 'design') {//已发布不存在删除按钮
        isShow = getButton(menus, 'design') 
      }else if(notShowDelete&&!exsitList?.toString()?.includes('update')){//删除 设计按钮都不存在 其他按钮可平铺
        isShow = true
      }else{
        isShow = false;
      }
            ...
            <div>
              <Space>
                ...
                {/* {getButton(menus, 'update') && (
                  <a onClick={onEdit.bind(this, record, 0)}>设计</a>
                )}
                {getButton(menus, 'delete') && record.isDeploy != 1 && (
                  <a onClick={onDelete.bind(this, record.formId, '')}>删除</a>
                )}
                {isShowFn(record, 'design') && (
                  <a onClick={onEditApp.bind(this, record, 0)}>APP设计</a>
                )}
                {isShowFn(record, 'view') && (
                  <a
                    onClick={onOperatingMoreClick.bind(
                      this,
                      { key: 'preview' },
                      record.formId,
                      record.mainVersion,
  ...
  function doImgUpload(options) {
    ...
    // })
    dispatch({
      type: 'formEngine/presignedUploadUrl',
      payload: {
          ...
          <Space>
            {/**
               <Dropdown overlay={moreMenu} trigger={['click']}>
                <Button className={styles.fontSize7}>在线设计表单<DownOutlined style={{verticalAlign: 'text-bottom'}}/></Button>
              </Dropdown>
              *
              */}
            ...
            >
                <Button className={styles.bt} >导入</Button>
            </Upload> */}
              {getButton(menus, 'design') && (
                <Button onClick={onLine.bind(this)}>在线设计</Button>
              )}
              {getButton(menus, 'design') && (
                <Button onClick={onBusinessForm.bind(this)}>引用设计</Button>
              )}
              {getButton(menus, 'design') && (
                <Button onClick={() => onPrint()}>打印设置</Button>
              )}
              {!currentNode.key ? (
                <Button
                  onClick={() => {
              ...
              <Button onClick={onExport.bind(this)}>导出</Button>
              <Button onClick={saveBelongOrg.bind(this)}>归属单位</Button>
            </div>
          </Space>
        </div>
  ...
  return (
    <div className={formStyle.add_Modal} id="formEngine_container">
      <ReSizeLeftRight
      ...
      {isModalPreview && (
        <ModalPreview
          ...
          version={mainVersion}
          formId={mainFormId}
          containerId="formEngine_container"
        />
      )}
      <ViewDetailsModal
        title="查看表单"
      ...
      {isShowRelationModal &&
          ...
          >
            <RelevanceModal nameSpace="formEngine" spaceInfo={
              ...
              originalData,
              selectNodeType,}}
                            orgUserType="ORG" containerId="formEngine_container"  />
...
export default connect(({ formEngine, user }) => ({
  ...formEngine,
  user,
}))(FormEngine);
