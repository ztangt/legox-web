...
export default async function request(action, options) {
  if (window.location.href.includes('/cloud/applyConfig')||window.location.href.includes('/cloud/designer/formDesigner')&&!window.localStorage.getItem('userToken_cloud')) {
  ...
  if((data.code=='419'||data.code=='403')&&window.localStorage.getItem('unset')!='true'){
    ...
    return new Promise(async(resolve, reject) => {
      ...
      ///cloud/applyConfig /cloud/designer/formDesigner获取的刷新时间不同
