  ...
  effects: {
    ...
    *getWorkSearch({payload,namespace,callback}, { call, put }) {
      ...
      if(data.code==200){
        ...
        let searchColumnCodes = typeof data.data.searchColumnCodes!='undefined'&&data.data.searchColumnCodes?data.data.searchColumnCodes.split(','):[];
        ...
        if(typeof callback=='function'){
          callback(searchColumnCodes,typeof data.data.listColumnCodes!='undefined'&&data.data.listColumnCodes?data.data.listColumnCodes.split(','):[]);
        }
    ...
    *getWorkRule({payload,namespace,callback},{call,put,select}){
      ...
      if(data.code==200){
        ...
        callback&&callback(workRules,workRules.length?workRules[0].groupCode:"",workRules.length?workRules[0].workRuleId:"")
        // yield put({
        //   type:`${namespace}/updateStates`,
        //   payload:{
        //     workRules:workRules,
