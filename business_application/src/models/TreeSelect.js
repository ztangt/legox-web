...
const  loop = (array,children,payload)=>{
  for(var i=0;i<array.length;i++){
    ...
    if(payload.parentId == array[i]['id']){
      ...
      if(array[i].children&&array[i].children.length!=0&&array[i]['children'][0].key!='-1'){
...
export default {
  ...
  effects: {
    *getOrgChildren({ payload,callback,state,extraParams}, { call, put }) {
      ...
      const { data } = yield call(apis.getOrgTree, payload, 'getOrgChildren', 'treeSelect');
      if (data.code == 200) {
        let sourceTree = extraParams.treeData ? extraParams.treeData : state.treeData
    ...
    *getSearchTree({ payload,callback }, { call, put }) {
      ...
      if (data.code == 200) {
        ...
        const { keys, array } = loop(data.data.jsonResult, state.expandedKeys, {})
