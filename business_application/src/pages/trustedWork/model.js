...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }, { call, select }) {
      history.listen((location) => {
        // if (location.pathname === '/trustedWork') {
        //   dispatch({
        //     type:'updateStates',
        //     payload:{
        //       limit:limit
  ...
  effects: {
    *getTrustedWork({payload,callback,setState},{call,put,select}){
      const {data} = yield call(apis.getTrustedWork,payload,'','trustedWork');
      if(data.code==200){
        setState({
    ...
    *traceWork({payload,params,setState,state},{call,put,select}){
      ...
      const {data} = yield call(apis.traceWork,payload,'','trustedWork');
      if(data.code==200){
