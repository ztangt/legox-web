...
function Work({dispatch,user,loading,location}){
  ...
  const traceWork=(bizInfoId,isTrace,obj)=>{
    ...
    getTrustedWork(searchWord,currentPage,limit,[],'')

  }
  //获取列表
  const getTrustedWork=(searchWord,start,limit,paramsJson,workRuleId)=>{
    ...
    dispatch({
      type:'trustedWork/getTrustedWork',
      payload:{
        searchWord,
        start,
        limit,
