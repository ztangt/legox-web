...
function Index({
  ...
  const getUserGroup=(searchWord)=>{
    console.log("searchWord",searchWord,"spaceInfo",spaceInfo)
    if(searchWord){
  ...
  console.log("originalData09",originalData,"treeData",treeData)
  //根据nodeType来判断节点是否禁掉 checkbox
  const loopTree = (tree, nodeType) => {
    tree.map((data) => {
      if(nameSpace == 'subProcessArrangeSpace'){
        if (nodeType == 'ORGANIZATION') {
          data['disableCheckbox'] = false;
        } else {
          if (data.orgKind == nodeType) {
            data['disableCheckbox'] = false;
          } else {
  ...
  const leftRender = (
    ...
    console.log("nodeType",newTreeData,"treeData",treeData,"orgUserType",orgUserType)
    return (
          ...
          <ITree
            ...
            nodeType={
              ...
              nameSpace === 'calendarMg'
                ? 'POST':nameSpace === 'subProcessArrangeSpace'?nodeType=='DEPT'?'ORG_':nodeType
                : nodeType
            }
            ...
            style={{}}
            isAddress={nameSpace == 'notification' ? true : ''}
  ...
  console.log("orgUserType111",orgUserType)
  return (
      ...
      {orgUserType == 'USER' ? (
        <ReSizeLeftRightCss
          ...
          leftChildren={leftRender(
            false,
            [],
            nameSpace === 'subProcessArrangeSpace'? 'ORG_':'DEPT',
            '请输入单位/部门名称',
            false,
          )}
          rightChildren={
