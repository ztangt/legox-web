...
lm.utils.copy( lm.LayoutManager.prototype, {
  ...
  // removeDragSource: function( element, itemConfig ) {
	// 	var dragSource = new lm.controls.DragSource( $( element ), itemConfig, this );
	// 	dragSource.destroy();
	// 	lm.utils.removeFromArray( dragSource, this._dragSources );
	// },
...
lm.utils.copy( lm.controls.Splitter.prototype, {
	...
	_createElement: function() {
		...
		if( this._isVertical ) {
			...
			dragHandle.css( 'height', this._size + handleExcessSize +1 );
			element.addClass( 'lm_vertical' );
			element[ 'height' ]( this._size );
		} else {
			dragHandle.css( 'left', -handleExcessPos );
			dragHandle.css( 'width', this._size + handleExcessSize +1 );
...
lm.items.Stack = function( layoutManager, config, parent ) {
	...
	// this.element = $( `<div class="lm_item lm_stack" style="min-height:${config?.content?.[0]?.minHeight || 400}px"></div>`);
