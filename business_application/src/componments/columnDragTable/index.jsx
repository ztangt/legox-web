...
class DragTable extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      height: 0,
      rowHeight: 40,
      // rowHeight: this?.tableRef?.current?.getElementsByClassName('oddRow')[0]?.clientHeight || 40,
    };
  }
  onResize() {
    const {
      ...
      () => {
        if (!closeReSize) {
          if (modulesName == 'setState') {
            setParentState({
              limit: Math.floor(this.state.height / this.state.rowHeight) || 10,
            });
          } else {
            dispatch({
  ...
  setBaseHeight() {
    ...
    if (!reqFlag) {
      if (modulesName == 'setState') {
        setParentState({
          limit: Math.floor((baseHeight-120) / this.state.rowHeight),
          currentHeight:
            baseHeight -
            8 -
            2 * TABLE_HEAD_HEIGHT -
      ...
      if (modulesName == 'setState') {
        ...
        dispatch({
          type: `${modulesName}/updateStates`,
          payload: {
            // limit: Math.floor(baseHeight / this.state.rowHeight),
            currentHeight:
              baseHeight -
              8 -
              2 * TABLE_HEAD_HEIGHT -
    ...
    if (nextProps.columns !== prevState.columns) {
      ...
      if (columns.length && typeof columns[0].title == 'string') {
        columns.forEach((element, index) => {
          ...
          //  
          if (nextProps.taskType === 'dynamic' && element.title !== '序号' && element.title !== '操作') {
            element.title = (
  ...
  render() {
    const { components, columns, disableDrap, taskType, ...others } =
      this.props;
