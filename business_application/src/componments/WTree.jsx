...
class TreeSelect extends React.Component {
  ...
  componentDidMount(){
    ...
    //默认左侧列表都是查询的ORG，然后查询单位下级的时候，如果查询的是部门那么传DEPT，查询的是单位传ORG
    this.init(nodeType=='DEPT'||nodeType=='ALL'?'ORG':nodeType,treeData,nodeIds,isAddress)
  ...
  getTreeData(nodeType,nodeIds,child,isAddress){
    const {dispatch,moudleName,isControl,treeType,type,location} = this.props
    if(isControl){
      dispatch({
        ...
        pathname: location.pathname,//当前路由路径
    ...
    onSelect(selectedKeys,info){
      parentNames = []
      deptNames = []
      const { dispatch, getData, moudleName ,location} = this.props
      const { treeData} = this.state;
      if(info.node){
          if(`${moudleName?moudleName:location.pathname.split("/")[1]}` == 'userInfoManagement'){
            ...
            dispatch({
              type: `${moudleName?moudleName:location.pathname.split("/")[1]}/updateStates`,
              payload:{
                currentNode: info.node
              }
            })
    ...
    onExpand(expandedKeys, {expanded, node}){
      const { dispatch,nodeType,moudleName ,location} = this.props
    ...
    loop(arr,expandedKeys){
      let self = this;
      const { dispatch, moudleName,location } = this.props
     ...
     */
    onSearchTree(value){
      const { nodeType,dispatch,moudleName,treeType,type,isControl,location } = this.props
      const { treeData} = this.state;
      ...
      if(isControl&&type!='CURRENTORG'&&type!='ORGS'&&type!='HIGHERORG'){//非当前单位 限定单位 上级单位
        dispatch({
          type: 'tree/getControlTree',
          payload:{
            searchWord: value,
            nodeIds: type=='ALLORGS'?'':treeData.map((item)=>{return item.nodeId}).join(','),
            treeType,
            type,
            child:0,
          },
    ...
    render(){
        const { plst,loading,checkedKeys,checkable,checkStrictly,isShowSearch,style,onAdd,isShowAdd,location} = this.props
        ...
        console.log('treeData=',treeData);
          return(
            ...
            >
              ...
              {location?.pathname!=='/addressBook'?<Spin
               ...
               >
                ...
                <Tree
                  ...
                  {//根节点编辑操作
                    ...
                    !node.parentId&&location.pathname=='/unitInfoManagement'?
                    <FormOutlined
