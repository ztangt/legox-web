...
function Index({
  ...
  // } = useModel('uploadfile')
  const getFileMD5=(payload,callback,nextState)=>{
    dispatch({
      type:"boxUploadfile/getFileMD5",
      payload,
      callback,
      nextState
    })
  }
  const presignedUploadUrl=(payload,callback,nextState)=>{
    dispatch({
      type:"boxUploadfile/presignedUploadUrl",
      payload,
      callback,
      nextState
    })
  }
  const getFileMerage=(payload,callback,nextState)=>{
    dispatch({
      type:"boxUploadfile/getFileMerage",
      payload,
      callback,
      nextState
    })
  }
  const storingFileInformation=(payload,callback,nextState)=>{
    dispatch({
      type:"boxUploadfile/storingFileInformation",
      payload,
      callback,
      nextState
    })
  ...
  const presignedUploadUrlCallback = (data, file, nextState) => {
    uploadStore(
      {
        url: `${data.presignedUploadUrl}`,
        file: file,
  ...
  //上传到minIO
  const uploadStore = async (payload, nextState) => {
  ...
  // 根据文件路径合并文件的回调
  const getFileMerageCallback = async (data, nextState) => {
