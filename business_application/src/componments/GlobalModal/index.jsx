import {
  ...
  import { useState,useEffect,useRef} from 'react';
    ...
    /**使用此组件无需使用bodyStyle等，可在项目中自行查找 
     * @footer 按照 antd footer={null}底部footer不设置其他无效
     * 外部传参根据宽度尺寸判断
     * @widthType 宽度类型 必需传入
     * @top 距离顶部高度
  ...
  const GlobalModal = (props)=>{
    ...
    useEffect(()=>{
        ...
        */
        const types = {
            ...
            page: ()=>{
                const has_padding = props.footer==null?0:MODAL_PADDING_FOOTER
                setLargeHeight(clientHeight-MODAL_PADDING_TOP-MODAL_PADDING_HEADER-has_padding);
            }
        }
        types[modal_type]()
   ...
   const getClient = ()=>{
        ...
        const getClient_ = ()=>{
            ...
            const top = (nowTop-getDynamicHeight(clientHeight).curHeight[props.widthType])/2-(MODAL_PADDING_HEADER+MODAL_PADDING_FOOTER)/2
            return {
                left,
                top,
                clientHeight,
   ...
   const getModalBody = ()=>{
    ...
    const footerChange = ()=>{
        ...
        <div><Button onClick={props.onCancel}>{props.cancelText||'取消'}</Button><Button onClick={props.onOk}>{props.okText||'确定'}</Button></div>
        return footer
    }
    // 计算content 弹窗内容
    ...
    return (
        <Modal style={styleChange()} wrapClassName='dragBox'{...props} getContainer={typeof props.getContainer=='function'?props.getContainer:()=>false} destroyOnClose={true} title={
            titleChange()
        }  footer={footerChange()} width={modalWidth[props.widthType]}
