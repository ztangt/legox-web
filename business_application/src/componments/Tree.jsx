...
class TreeSelect extends React.Component {
  ...
  componentDidMount() {
    const { nodeType, treeData, nodeIds,moudleName, isAddress, treeExcuteAuth, setParentState,stateObj } =
      this.props;
    //左侧列表都是查询的ORG，然后查询单位下级的时候，如果查询的是部门那么传DEPT，查询的是单位传ORG
    this.init(
      nodeType == 'DEPT' ? moudleName=='subProcessArrangeSpace'?'POST':'ORG' : nodeType,
      treeData,
      nodeIds,
      isAddress,
      treeExcuteAuth,
  ...
  init(nodeType, treeData, nodeIds, isAddress, onlySubDept,setParentState,
    stateObj,) {
      const {moudleName} = this.props
      if(moudleName=='subProcessArrangeSpace'){
        this.getTreeData(nodeType, nodeIds, 0, isAddress, onlySubDept,setParentState,
          stateObj);
      }else{
  ...
  getTreeData(nodeType, nodeIds, child, isAddress, onlySubDept,setParentState,
    stateObj,) {
    const { dispatch, moudleName, isControl, treeType, type, treeExcuteAuth,  } =
      this.props;
      console.log("moudleName009",moudleName)
    if (isControl) {
      dispatch({
        ...
        dispatch({
          ...
          payload: {
            ...
            onlySubDept: onlySubDept
              ? 0
              : treeExcuteAuth == 'A0002' || treeExcuteAuth == 'A0004'
              ? 1
              : 0
          },
          pathname: history.location.pathname, //当前路由路径
  ...
  onExpand(expandedKeys, { expanded, node }) {
    ...
    if (expanded) {
      ...
      if (node.isParent == 1) {
        ...
        }else if(moudleName == 'subProcessArrangeSpace'){
          dispatch({
            type: 'tree/getOrgTree',
            payload:{
              parentId:node.key,
        ...
        dispatch({
          ...
          callback: (data) => {
            if(setParentState){
              setParentState({
                treeData: data?.array,
                expandedKeys: Array.from(new Set(data?.keys)),
              })
            }
          },
        });
