import { Input, Form, Table, Select, Row, Col, DatePicker, Popover, Checkbox, message, Dropdown, Menu } from 'antd';
...
import { TASKSTATUS, BIZSTATUS, TODOBIZSTATUS, ALLTASKSTATUS } from '../../service/constant.js';
import { ORDER_WIDTH, BASE_WIDTH } from '../../util/constant'
import OneKeyApprove from '../../pages/index/componments/oneKeyApprove';
...
function List({ dispatch, loading, getListData, setParentState, stateObj, taskType, isShowWorkRule,
  rowKey, oprationRenderHtml, placeholder, oprationCol, showMakeAction, goFormUrlProps, defaultColumns,
  extraOprationFn, location, user }) {
  const { menuObj } = user;
  const pathname = location.pathname;
  const uuId = location?.query?.uuId || '';
  const allWorkSearchValue=location.query?.searchValue
  const { limit, returnCount, currentPage, searchWord, list, searchColumnCodes, paramsJson, oldSearchColumnCodes,
    workRules, listColumnCodes, selectedRowKeys, listTitle, workRuleId, groupCode, currentHeight,selectedRows } = stateObj;
  ...
  useEffect(() => {
    ...
    } else if (taskType == 'CATEGORY' || taskType == 'MONITOR') {//个人归档没有事项规则
      dispatch({
        type: 'work/getWorkSearch',
        payload: {
          taskType,
    ...
    if (Object.keys(listTitle).length&&limit) {
      let newColumns = taskType == 'TODO' ? [
        {
          ...
          align: 'left',
          fixed: true,
          render: (value, obj, index) => <div onClick={goFormUrl.bind(this, obj)} style={{ minWidth: "80px" }}>{index + 1}</div>
        },
        {
          title: '预警',
          dataIndex: 'warning',
          ...
          width: ORDER_WIDTH,
          render: (text) => {
            return <span className={styles.warnColor} style={{ background: text == 'green' ? '#50AC50' : text == 'red' ? '#FA2C19' : text == 'grey' ? '#AAAFBD' : text == 'yellow' ? '#EA9743' : '' }}></span>
          }
        }
      ] : [{
        title: "序号",
      ...
      Object.keys(listTitle).map((key) => {
        ...
        newColumns.push({
          ...
          key: `${key}`,
          ellipsis: true,
          width: key == 'bizTitle' ? BASE_WIDTH * 2.5 : BASE_WIDTH,
          render: (text, obj) => {
            if (key == 'bizTitle') {
              if (pathname == '/waitMatter') {
                return (
                  <div
                    onClick={typeof goFormUrlProps != 'undefined' ? goFormUrlProps.bind(this, obj) : goFormUrl.bind(this, obj)}
                    style={{ minWidth: minWidth, display: "flex" }}
                  >
                    {obj.taskStatus == '0' ? <div style={{ color: 'red', marginRight: '2px', display: 'inline-block' }}>
                      {/* <MyIcon type="iconnew" style={{fontSize:'18px'}}/> */}
                      <img src={new_img} />
                    </div> : ''}
                    <a title={text} style={{ display: 'inline-block' }} className={styles.noticeTitle}>{text}</a>
                    <div style={{ marginLeft: '2px', display: "flex" }}>{showMakeAction(obj.makeAction)}</div>
                  </div>
                )
              } else {
                return (<a onClick={goFormUrl.bind(this, obj)} title={text} style={{ minWidth: minWidth }} className={styles.noticeTitle}>{text}</a>)
              }
            } else if (key.includes('Time')) {
              return (<div style={{ minWidth: minWidth }}>{text ? dataFormat(text, 'YYYY-MM-DD HH:mm:ss') : ''}</div>);
        ...
        return (
          <Fragment key={key}>
            <Col span={width} key={key}>
              ...
              >
                <DatePicker showTime={{ defaultValue: moment('00:00:00', 'HH:mm:ss') }} format="YYYY-MM-DD HH:mm:ss" style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col span={width} key={key}>
              <Form.Item
  ...
  const getSearchFields = (width = 8) => {
    ...
    searchColumnCodes.map((item, key) => {
      ...
      children.push(
        <Fragment key={key}>
          {componentRender(item, key,width)}
        </Fragment>
      );
    })
  ...
  const allSearchFileld = () => {
    return (
      <div className={styles.search_col_pop}>
        ...
        <br/> */}
        <Checkbox onChange={onCheckAllChange} indeterminate={indeterminate} checked={checkAll}>全选</Checkbox>
        <Checkbox.Group
          style={{ width: '100%' }}
        ...
        >
          {searchCol.filter((item, index) => item.key !== "ACT_NAME").map((item, index) => {
            if (item.type.includes(taskType)) {
              return (
        ...
        <div className={styles.p_opration}>
          ...
          <Button onClick={saveSearchCol.bind(this)} type="primary">保存</Button>
        </div>
      </div>
    )
  }
  ...
  const workRuleMenu = (
    <Menu>
      {isShowWorkRule && workRules.map((item) => {
        return (<Menu.Item onClick={() => { setWorkRuleIdFn(item.workRuleId, item.groupCode) }} key={item.workRuleId}>
          <span>
            {item.groupName}
          </span>
        </Menu.Item>)
  ...
  return (
    <>
      <div className={styles.work_warp} id={`modal_biz_sol_${taskType}`}>
        <div ref={ref} id={`list_head-${taskType}-${location.pathname}`}>
          {taskType == 'CIRCULATE' &&
            <div className={isShowHighSearch ? styles.button_right : styles.pos}>
              <Button type="primary" onClick={setCirculate}>设为已阅</Button>
            </div>
          }
          {taskType == 'TODO' &&
            <div className={isShowHighSearch ? styles.button_right : styles.pos}>
              <Button type="primary" onClick={oneKeyApproveOpen}>一键审批</Button>
            </div>
          }
          {oprationRenderHtml}
          {isShowWorkRule &&
            <div className={styles.work_rule}>
              <Dropdown overlay={workRuleMenu}>
                <div onClick={e => e.preventDefault()}>
                  {/* <span>业务</span> */}
                  <div>
                    <span className={styles.select_rule}>{workRuleId && workRules.length ? workRules.filter((item) => item.workRuleId == workRuleId)[0].groupName : ""}</span>
              ...
              <>
                ...
                <div className={styles.f_opration} id="set_opration">
                  ...
                  <Popover
                    ...
                    getPopupContainer={() => document.getElementById(`modal_biz_sol_${taskType}`)}
                  >
                  ...
                  <Button className={styles.o_pack} onClick={() => { packUp()}}>收起</Button>
                </div>
              </>
              :
              <>
                ...
                />
                <span onClick={() => { showAdvancedSearch()}}>
        ...
        <div className={styles.workTable} style={{ height: `calc(100% - ${height}px - 32px)` }}>
          {
            taskType == 'CIRCULATE' || taskType == 'CATEGORY' || taskType == 'ALL' || taskType == 'MONITOR'||taskType == 'TODO' ?
              <ColumnDragTable
                // listHead={`list_head`}
          ...
          {taskType != 'CATEGORY' && taskType != 'MONITOR' ? <SetCol
        ...
        <IPagination
          ...
          total={returnCount}
          isRefresh={true}
          refreshDataFn={() => { getListData('', 1, limit, paramsJson, groupCode == 'R0000' ? '' : workRuleId) }}
        />
        {taskType == 'TODO' && <div style={{ width: '30%', position: 'absolute', bottom: 16 }} className={styles.warnMessage}><span>预警提示</span><span><i></i>正常</span><span><i></i>即将超期</span><span><i></i>已超期</span><span><i></i>无预警</span></div>}
        </div>
      </div>
      {oneKeyApprove.show ? <OneKeyApprove onClose={oneKeyApproveClose} data={oneKeyApprove.data} modalType={'page'} /> : null}
    </>
