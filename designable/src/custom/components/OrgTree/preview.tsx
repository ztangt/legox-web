...
export const OrgTree: DnFC<InputProps> = observer((props) => {
...
OrgTree.Behavior = createBehavior({
  name: 'OrgTree',
  extends: ['Field'],
  selector: (node) => node.props['x-component'] === 'OrgTree', //组件
  designerProps: {
    droppable: true,
    propsSchema: createFieldSchema(AllSchemas.OrgTree),
  },
  designerLocales: AllLocales.OrgTree, //语言
})
//createResource 创建资源基础信息，用于左侧拖拽组件
OrgTree.Resource = createResource({
  icon: <MyIcon type="icon-danweishu" className="custom-icon" />,
  elements: [
    {
      componentName: 'Field',
      props: {
