...
export const PortalBar: DnFC<PortalBarProps> = observer((props) => {
  ...
  let defaultProps = {
    option: {
      ...
      xAxis: {
        type: 'category',
        data: ['部门A', '部门B', '部门C', '部门D', '部门E', '部门F'], // 横轴
      },
      yAxis: {
        type: 'value',
        splitLine: {
...
PortalBar.Behavior = createBehavior({
  name: 'PortalBar',
  extends: ['Field'],
  selector: (node) => node.props['x-component'] === 'PortalBar',
  designerProps: {
    propsSchema: creatPortalFieldSchema(AllSchemas.PortalBar),
  },
  designerLocales: AllLocales.PortalBar,
})

PortalBar.Resource = createResource({
