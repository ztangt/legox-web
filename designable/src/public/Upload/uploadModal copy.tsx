...
interface State {
  ...
  nowMessage: string
  isCancel: boolean
  merageFilepath: string
  fileStorageId: string
  htmlFileStorageId: string
  fileNames: any
  typeNames: any
  optionFile: any
  fileList: any
...
function Index({
  ...
  const getFileMD5Callback = async (
    ...
    setState({ getFileMD5Message: data })
    // new
    ...
    if (isPresigned) {
      ...
      if (data.have === 'Y') {
        ...
        setState({
          ...
          md5FileId: data.fileId,
          md5FilePath: data.filePath,
          needfilepath: data.filePath,
        })
        // 返回值have=N  ？不存在
        ...
        uploadStore(
          ...
          { ...nextState, needfilepath: filePath }
        )
      }
      ...
      // 存在，返回对应minio路径及主键
      if (data.data.have === 'Y') {
        uploadSuccess && uploadSuccess(data.filePath, '', state.optionFile)
        setState({
          md5FileId: data.fileId,
          md5FilePath: data.filePath,
          fileExists: true,
      ...
      if (fileChunkedList.length && fileChunkedList.length - 1 != index) {
        ...
        if (fileSize > 40 * 1024 * 1024) {
          getFileMerage(
            ...
            { ...nextState, isStop: true, isContinue: true, isCancel: true }
          )
        } else {
          storingFileInformation(
            {
  ...
  const getFileMerageCallback = async (data: any, nextState: any) => {
    ...
    storingFileInformation(
      ...
      storingFileInformationCallback,
      { ...nextState, merageFilepath: data, isStop: true, isContinue: true }
    )
  }
