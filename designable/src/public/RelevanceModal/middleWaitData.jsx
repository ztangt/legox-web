...
function MiddleUserAccredit({
  ...
  const showIdentity = (item) => {
    ...
    let mainPost =
      item.isMainPost == 1 ? '主岗' : item.isMainPost == 0 ? '兼职' : ''
    let leavePost =
      item.isLeavePost == 1 ? '离岗' : item.isLeavePost == 0 ? '在岗' : ''
    // 使用数组过滤和join方法拼接字符串
  ...
  const Row = ({ index, style }) => {
    ...
    return (
      <div style={style} key={item[idKey] ? item[idKey] : `un_${index}`}>
        <Checkbox
          value={item[idKey]}
          onChange={changeSelect}
          checked={
            selectIds && selectIds.filter((i) => i == item[idKey]).length > 0
              ? true
              : false
          }
  ...
  return (
    <div className={styles.await_select_list}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.middle_list} ref={listRef}>
