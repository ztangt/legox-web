...
const toDesignableFieldProps = (
  ...
  //   schema?.['x-display'] &&
  //   schema?.['x-display'] != 'visible'
  //     ? '隐藏字段'
  //     : schema['x-component-props']?.placeholder
  ...
  // results.title = results.title && (//TODO 暂时先注释掉
...
export const Field: DnFC<ISchema> = observer((props) => {
  ...
  const fieldProps = {
    ...
    pattern:
      (useWorkbench()?.type == 'DESIGNABLE' &&
        props?.['x-component'] == 'Invoice') ||
      props?.['x-component'] == 'PullData' ||
      props?.['x-component'] == 'TreeTable' ||
      props?.['x-component'] == 'AttachmentBiz' ||
      props?.['x-component'] == 'UploadFile'
        ? 'readOnly'
        : props?.['x-pattern'],
    disabled:
      useWorkbench()?.type == 'DESIGNABLE' &&
