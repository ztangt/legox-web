...
import { BASE_WIDTH } from '../../service/constant'

// 宽度可伸缩render
const ResizableTitle = (tabelProps, props, e) => {
  console.log('props===', tabelProps)
  console.log('e===', e)
...
class DragTable extends React.Component {
    ...
    this.setState(
      ...
      () => {
        ...
        dispatch({
          ...
          payload: {
            limit: Math.floor(this.state.height / 40),
            currentHeight:
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (nextProps.columns !== prevState.columns) {
      ...
      if (columns && columns?.length && typeof columns[0].title == 'string') {
        columns.forEach((element) => {
          element.title = (
            <p className="dragHandler">
              {element?.isRequired ? <i className={styles.reuired_i}>*</i> : ''}
              {element.title}
            </p>
          )
  ...
  components = {
    header: {
      cell: ResizableTitle.bind(this, this.props),
    },
  }
