...
// ]
function Look(props) {
  const { setIsShowLookModal, getDetailInfo } = props
  ...
  useEffect(() => {
    if (props.isMultiple) {
      getDetailInfo && getDetailInfo(callbackInfo, 1, 10)
    } else {
      getDetailInfo && getDetailInfo(callbackInfo, 1, 200)
    }
  }, [])
  ...
  const changePage = (nextPage: number, size: number) => {
    setPageSize(size)
    getDetailInfo && getDetailInfo(callbackInfo, nextPage, size)
  }
  return (
    <Modal
      visible={true}
...
Look.propTypes = {
  ...
  setIsShowLookModal: PropTypes.func,
