...
function Index({
    ...
    const beforeUpload = (file, fileList) => {
        ...
        if (allowFileType) {
            ...
            message.error(
                `上传的文件不能大于${
                    requireFileSize >= 1024 ? (requireFileSize / 1024).toFixed(0) + 'GB!' : requireFileSize + 'MB!'
                }`,
            );
            ...
            fileReader.onload = (e) => {
                // console.log(e.target.result, '21212');
                    ...
                    if (nameSpace === 'dynamicPage') {
                        dispatch({
                            ...
                            payload: {
                                ...
                                filePath: filePath
                                    ? `${filePath}/${file.name}`
                                    : `monitor/${dataFormat(String(new Date().getTime()).slice(0, 10), 'YYYY-MM-DD')}${
                                          typeName == '' ? '' : '/' + typeName
                                      }/${file.name}`,
                                file: file,
                            },
    ...
    return (
        <Upload
            ...
            multiple={nameSpace == 'notification'}
        >
            {buttonContent ? buttonContent : <Button type={type}>选择文件</Button>}
        </Upload>
    );
}
