...
class TreeSelect extends React.Component {
    ...
    onSelect(selectedKeys, info) {
        ...
        const { treeData } = this.state;
        if (info.node) {
            if (`${moudleName ? moudleName : history.location.pathname.split('/')[1]}` == 'userInfoManagement') {
                ...
                dispatch({
                    type: `${moudleName ? moudleName : history.location.pathname.split('/')[1]}/updateStates`,
                    payload: {
                        currentNode: info.node,
                    },
                });
     ...
     */
    onSearchTree(value) {
        const { nodeType, dispatch, moudleName, treeType, type, isControl } = this.props;
        const { treeData } = this.state;
    ...
    render() {
        const { plst, loading, checkedKeys, checkable, checkStrictly, isShowSearch, style, onAdd, isShowAdd } =
            this.props;
        ...
        return (
            <div className={styles.tree}>
                ...
                {isShowAdd && (
                    <Button onClick={onAdd.bind(this)} type="primary" style={{ width: 205, marginTop: 10 }}>
                        添加根节点
                    </Button>
                )}
                        ...
                        titleRender={(node) => (
                            <span key={node.key} className={styles.tree_node}>
                                ...
                                {node.title}
                                {
                                    ...
                                    !node.parentId && history.location.pathname == '/unitInfoManagement' ? (
                                        <FormOutlined
