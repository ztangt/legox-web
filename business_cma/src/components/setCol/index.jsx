 ...
 */
import { ArrowDownOutlined, ArrowUpOutlined, EllipsisOutlined, SettingOutlined } from '@ant-design/icons';
...
function SetCol({ changeColVisiblePop, taskType, colVisiblePop, saveCols, allCols, selectColumnCode, id }) {
    ...
    useEffect(() => {
        if (colVisiblePop) {
            ...
            selectColumnCode.forEach((item) => {
                let columnsItem = allCols.find((columnsItem) => columnsItem.key == item);
                if (columnsItem) {
                    selectList.push(columnsItem);
                }
            });
            let noSelectList = allCols.filter((item) => !selectColumnCode.includes(item.key));
            ...
            setIndeterminate(!!selectColumnCode.length && selectColumnCode.length < allCols.length);
        }
    }, [colVisiblePop]);
    ...
    const allColFileld = (cols) => {
        return (
            <div className={styles.search_col_pop}>
                ...
                <Checkbox.Group value={colSelectKey} className={styles.col_check} onChange={onChangeCheck}>
                    {cols.map((item, index) => {
                        return (
                            <div className={styles.setCol_rows} key={item.key}>
                                <div className={styles.col_row}>
                                    <EllipsisOutlined rotate={90} />
                                    <EllipsisOutlined rotate={90} style={{ marginLeft: '-8px' }} />
                                    <Checkbox value={item.key}>{item.title}</Checkbox>
                                    <div className={styles.move_col_box} style={{ width: '30px' }}>
