...
const ReSizeLeftRight = (props) => {
     ...
     */
    const vResizeOver = (e) => {
        let containerWidth = document.getElementById(`designable_v_resize_container_${props.level}`).offsetWidth;
        const vRigthNumLimit = props.vRigthNumLimit; //最小的右侧宽度
        const hEle = document.getElementById(`designable_h_resize_container_${props.level}`);
        let resizeOffsetInfo = getEleOffset(hEle);
        if (isVResize && vNum >= props.vLeftNumLimit && containerWidth - vNum >= vRigthNumLimit) {
    ...
    return (
        <div
            ...
            id={`designable_v_resize_container_${props.level}`}
        >
            <div
                className={styles.left}
                style={{ width: vNum - 2 }}
                id={`designable_h_resize_container_${props.level}`}
            >
                {props.leftChildren}
            </div>
            <div style={{ left: vNum + 20 }} draggable={false} onMouseDown={vResizeDown} className={styles.v_resize}>
                {/* <b className={styles.drag_bt}>
           <LeftOutlined />
         </b> */}
            </div>
            <div style={{ marginLeft: vNum + 14 }} className={styles.tree_dom}>
                {props.rightChildren}
            </div>
        </div>
    );
