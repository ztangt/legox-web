const { getOrgChildren, getSearchTree, getDepts, getPosts, getControlTree, getControlSearchTree } = apis;
...
export default {
    ...
    effects: {
        *getOrgChildren(
            { payload, callback, nodePath, currentNodeType, currentNodeId, curNodeName },
            { call, put, select },
        ) {
            ...
            try {
                const { data } = yield call(getOrgChildren, { ...payload }, '', 'treeState');
                ...
                if (data.code == REQUEST_SUCCESS) {
                    ...
                    if (payload.nodeType == 'POST' && payload.nodeId) {
                        //岗位和子集合并
        ...
        *getSearchTree({ payload, callback }, { call, put, select }) {
            ...
            try {
                const { data } = yield call(getSearchTree, payload, '', 'treeState');
                if (data.code == REQUEST_SUCCESS) {
        ...
        *getControlTree(
            ...
            const { postList } = yield select((state) => state.treeState);
            try {
                payload.nodeId = payload.orgIds;
                const { data } = yield call(getControlTree, { ...payload }, '', 'treeState');
                const loop = (array, children, org) => {
                    for (var i = 0; i < array.length; i++) {
                        // array[i]['title'] = `${NODE_TYPE[array[i]['nodeType']]}-${array[i]['nodeName']}`
                        array[i]['title'] = `${array[i]['nodeName']}`;
        ...
        *getControlSearchTree({ payload, callback }, { call, put, select }) {
            const expandedKeys = payload.orgIds;
            // delete payload.expandedKeys;
            payload.orgIds = payload.orgIds.join(',');
            try {
                const { data } = yield call(getControlSearchTree, payload, '', 'treeState');
                if (data.code == REQUEST_SUCCESS) {
                    const loop = (array, keys, org) => {
                        for (var i = 0; i < array.length; i++) {
                            //array[i]['title'] = `${NODE_TYPE[array[i]['nodeType']]}-${array[i]['nodeName']}`
        ...
        *getDepts({ payload }, { call, put, select }) {
            try {
                const { data } = yield call(getDepts, payload, '', 'treeState');
        ...
        *getPosts({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(getPosts, payload, '', 'treeState');
                if (data.code == REQUEST_SUCCESS) {
