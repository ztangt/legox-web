...
export default {
    ...
    effects: {
        *getList({ payload, callback }, { call, put, select }) {
            const { menuId } = yield select((state) => state.budgetTarget);
            try {
        ...
        *getTableList({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(apis.getBudgetProjectTree, payload, 'getTableList', 'budgetTarget');
                if (data.code == 200) {
                    yield put({
                        type: 'updateStates',
                        payload: {
        ...
        *getDetailList({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(apis.getNormHistory, payload, 'getDetailList', 'budgetTarget');
                if (data.code == 200) {
                    yield put({
                        type: 'updateStates',
                        payload: {
        ...
        *checkNorm({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(apis.checkNorm, payload, '', 'budgetTarget');
                if (data.code == 200) {
        ...
        *delFormData({ payload, callback }, { call, put, select }) {
            ...
            const { data } = yield call(apis.delFormData, payload, '', 'budgetTarget');
            if (data.code == 200) {
        ...
        *powerNorm({ payload, callback }, { call, put, select }) {
            ...
            payload.headers = cutomHeaders; //需要在headers中添加参数
            try {
                const { data } = yield call(apis.powerNorm, payload, '', 'budgetTarget');
                if (data.code == 200) {
                    callback && callback();
                } else {
                    message.error(data.msg);
        ...
        *annualCarryForward({ payload, callback }, { call, put, select }) {
            ...
            payload.headers = cutomHeaders; //需要在headers中添加参数
            try {
                const { data } = yield call(apis.annualCarryForward, payload, '', 'budgetTarget');
                if (data.code == 200) {
        ...
        *getBudgetProjectTree({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(apis.getBudgetProjectTree, payload, '', 'budgetTarget');
                const { treeData } = yield select((state) => state.budgetTarget);
        ...
        *getProjectBizSolId({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(apis.getProjectBizSolId, payload, '', 'budgetTarget');
                if (data.code == 200) {
                    callback && callback(data.data);
                } else {
                    message.error(data.msg);
        ...
        *exportFile({ payload, callback }, { call, put, select }) {
            const { cutomHeaders, menuId } = yield select((state) => state.budgetTarget);
            payload.headers = { ...cutomHeaders, menuId }; //需要在headers中添加参数
            const { data } = yield call(apis.more_budgetTarget_exportFile, payload, '', 'budgetTarget');
            if (data.code == 200) {
                callback && callback(data.data);
            } else {
                message.error(data.msg);
        ...
        *importExcel({ payload, callback }, { call, put, select }) {
            ...
            const { data } = yield call(apis.importExcel, payload, '', 'budgetTarget');
            if (data.code == 200) {
        ...
        *refreshImportExcel({ payload, callback }, { call, put, select }) {
            ...
            const { data } = yield call(apis.refreshImportExcel, payload, '', 'budgetTarget');
            if (data.code == 200) {
        ...
        *getDictList({ payload, callback }, { call, put, select }) {
            ...
            let keyList = [
                { key: 'NORMSTATE', value: 'normStateList' }, //指标状态
            ];
            for (let i = 0; i < keyList.length; i++) {
                ...
                if (!dictInfo) {
                    ...
                    try {
                        const { data } = yield call(apis.getBaseDataCodeTable, params);
                        if (data.code == 200) {
        ...
        *getState({ payload, callback }, { call, put, select }) {
            const data = yield select((state) => state.budgetTarget);
