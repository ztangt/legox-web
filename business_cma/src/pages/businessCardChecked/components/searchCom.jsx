...
const Index = ({ dispatch, getList, businessCardChecked, onExport, onBatchHandle, number, changeFormInfo }) => {
    const { companyList, backAccountList, orgList } = businessCardChecked;
    ...
    const changeOrg = (value) => {
        if (value) {
            dispatch({ type: 'businessCardChecked/getBankAccount', payload: { orgId: value } });
    ...
    return (
        <div id="businessCardChecked_head_id" className="pt10">
...
export default connect(({ businessCardChecked }) => ({ businessCardChecked }))(Index);
