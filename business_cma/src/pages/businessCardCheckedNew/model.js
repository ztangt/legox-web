...
export default {
    namespace: 'businessCardCheckedNew',
    state: {
        orgList: [], //管理单位
    ...
    effects: {
        ...
        *findLoginUserByIdAndRoleId({ payload, callback }, { call, put, select }) {
            const { data } = yield call(findLoginUserByIdAndRoleId, payload, '', 'businessCardCheckedNew');
            if (data.code == 200) {
        ...
        *getBankAccount({ payload, callback }, { call, put, select }) {
            const { data } = yield call(getAccountInfoList, payload, 'getBankAccount', 'businessCardCheckedNew');
            if (data.code == 200) {
                let list = data.data?.list || [];
                yield put({
                    type: 'updateStates',
        ...
        *onExport({ payload, callback }, { call, put, select }) {
            const { data } = yield call(BusinessCardCheckedExport, payload, 'onExport', 'businessCardCheckedNew');
            yield put({ type: 'updateStates', payload: { loading: false } });
            if (data.code == 200) {
        ...
        *batchCheck({ payload, callback }, { call, put, select }) {
            const { data } = yield call(batchCheck, payload, 'batchCheck', 'businessCardCheckedNew');
            yield put({ type: 'updateStates', payload: { loading: false } });
            if (data.code == 200) {
