...
import styles from '../../../components/works/list.less';
...
function WaitMatter({ dispatch, loading, location, menuObj }) {
    ...
    const getTodoWork = (searchWord, start, limit, paramsJson, workRuleId) => {
        ...
        if (location.pathname == '/meteorological') {
            pathname = `/${location.query?.microAppName}/${location.query?.url}`;
        }
        const info = menuObj[pathname];
