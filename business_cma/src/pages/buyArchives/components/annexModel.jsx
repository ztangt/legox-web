...
function DetailModel({ dispatch, buyArchives, mainTableId, bizInfoId }) {
    const { annexList, contractId } = buyArchives;
    ...
    const uploadSuccess = (filePath, fileId, file, fileFullPath) => {
        // console.log(mainTableId, 1111111,'传过来的mainTableId');
        if (!mainTableId) {
            return message.error('请检查节点ID是否存在');
        }
        ...
        dispatch({
            type: 'buyArchives/getBizRelAtt',
            payload: {
                bizInfoId: bizInfoId, //取主页列表的，purchaseRequestBizId有值的话取purchaseRequestBizId，没有的话取purchaseProjectBizId
                atts: JSON.stringify(atts),
                bizSolId: info.bizSolId,
                mainTableId: mainTableId, //取主页列表的，purchaseRequestMianId有值的话取purchaseRequestMianId，没有的话取purchaseProjectMainId
            },
            callback: () => {
                message.success('上传成功');
                dispatch({
                    type: 'buyArchives/getAnnexList',
                    payload: { purchasePlanId: contractId },
                });
            },
        });
    };
    ...
    return (
        <GlobalModal
            ...
            open={true}
            getContainer={() => {
                return document.getElementById('buyArchives_id');
            }}
            footer={[
