...
const { Option } = Select;
function SearchDom({ dispatch, contractUnstamped }) {
    ...
    //重置
    const onReset = async () => {
        console.log(getExportJson(), '---->getExportJson');
        form.resetFields();
    ...
    const [auditStatusTldt, setAuditStatusTldt] = useState(''); //验收状态
    //头部搜索,保存表单数据
    const onSearch = (searchWord, auditStatusTldt) => {
        dispatch({
            type: 'contractUnstamped/updateStates',
            payload: {
                formData: { searchWord, auditStatusTldt },
            },
        });
        getList(1, { searchWord, auditStatusTldt });
    };
            ...
            <div className={'flex pl8 pt8 flex_justify_between  flex_align_start'}>
                <div className={'flex mr20 flex_align_center mb8'}>
                    <Input.Search
                        ...
                        onSearch={(value) => onSearch(value, auditStatusTldt)}
                    />
                    <Select
                        style={{ width: '240px' }}
                        placeholder="请选择审核状态"
                        allowClear
                        onChange={(val) => setAuditStatusTldt(val)}
                        ...
                        <Row>
                            ...
                            <Col span={8}>
                                <Form.Item label="办理状态" name="bizStatus">
                                    <Select placeholder="请选择办理状态" allowClear options={statusList} />
                                </Form.Item>
                            </Col>
                        </Row>
