...
const Index = ({ dispatch, incomeDeposit }) => {
    const { limit, accountTypeList, orgList, accountList } = incomeDeposit;
    // 时间选择
    ...
    const onSearch = () => {
        ...
        dispatch({ type: 'incomeDeposit/updateStates', payload: { formInfo: { ...values }, loading: true } });
        dispatch({ type: 'incomeDeposit/getList', payload: { ...values, start: 1, limit } });
    };
    ...
    const changeOrg = (value) => {
        if (value) {
            dispatch({ type: 'incomeDeposit/getBankAccount', payload: { orgId: value } });
            form.setFieldValue('payerAccountNumber', '');
        }
    };
    return (
        <div id="incomeCollect_list_head" className={styles.incomeCollect_list_head}>
...
export default connect(({ incomeDeposit }) => ({
    incomeDeposit,
}))(Index);
