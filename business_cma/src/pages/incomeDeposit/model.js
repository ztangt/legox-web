...
const {
    getExtractList,
    getincomeDepositDetail,
    saveincomeDeposit,
    findLoginUserByIdAndRoleId,
    more_deposit_saveBalance,
    more_deposit_findBalanceDetail,
} = apis;

export default {
    namespace: 'incomeDeposit',
    state: {
        limit: 0,
        allPages: 0,
        returnCount: 0,
    ...
    effects: {
        ...
        *getBankAccount({ payload, callback }, { call, put, select }) {
            const { data } = yield call(apis.getAccountInfo, payload, 'getBankAccount', 'incomeDeposit');
            if (data.code == 200) {
                let list = data.data?.list || [];
                yield put({
                    type: 'updateStates',
        ...
        *getList({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(getExtractList, payload, 'getList', 'incomeDeposit');
                yield put({
                    type: 'updateStates',
                    payload: {
                        loading: false,
        ...
        *getDetailList({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(more_deposit_findBalanceDetail, payload, 'getDetailList', 'incomeDeposit');
                if (data.code == 200) {
        ...
        *saveDepositList({ payload, callback }, { call, put, select }) {
            try {
                const { data } = yield call(more_deposit_saveBalance, payload, 'saveDepositList', 'incomeDeposit');
                if (data.code == 200) {
        ...
        *getDictList({ payload, callback }, { call, put, select }) {
            ...
            for (let i = 0; i < keyList.length; i++) {
                ...
                if (!dictInfo) {
                    ...
                    try {
                        const { data } = yield call(apis.getBaseDataCodeTable, params, 'getDictList', 'incomeDeposit');
                        if (data.code == 200) {
