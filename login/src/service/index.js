...
const gen = (apiName,key)=>{
   ...
   * serviceName 微服务名
   */
  return function(payload,typeName,namespace,serviceName){
    ...
    if(actionArray.length>0&&serviceName){//替换微服务名
    ...
    if(method === 'GET'){
      if(action == 'sys/tenant/mark'||
        action == 'sys/tenant/config/login'){
          options.headers = {
            "TL-micro-app":microApp
          }
      }else{
    ...
    if(window.localStorage.getItem('dataRuleCode') && window.localStorage.getItem('dataRuleCode') != 'null'){
      options.headers = {
        ...options.headers,
