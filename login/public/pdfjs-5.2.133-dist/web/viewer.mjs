...
class PasswordPrompt {
  ...
  async open() {
    ...
    this.#activeCapability = createDeferred();
    try {
      await this.overlayManager.open(this.dialog);
    } catch (ex) {
...
class PDFAttachmentViewer extends BaseTreeViewer {
  ...
  reset(keepRenderedCapability = false) {
    ...
    if (!keepRenderedCapability) {
      this._renderedCapability = createDeferred();
    }
...
class PDFDocumentProperties {
  ...
  #reset() {
    this.pdfDocument = null;
    this.#fieldData = null;
    this._dataAvailableCapability = createDeferred();
...
class PDFFindController {
  ...
  #reset() {
    ...
    clearTimeout(this._findTimeout);
    this._findTimeout = null;
    this._firstPageCapability = createDeferred();
  }
  get #query() {
    const {
      query
...
class PDFOutlineViewer extends BaseTreeViewer {
  ...
  _dispatchEvent(outlineCount) {
    this._currentOutlineItemCapability = createDeferred();
  ...
  async _getPageNumberToDestHash(pdfDocument) {
    ...
    this._pageNumberToDestHashCapability = createDeferred();
...
class PDFScriptingManager {
  ...
  async dispatchWillPrint() {
    ...
    this.#willPrintCapability = createDeferred();
    try {
      await this.#scripting.dispatchEventInSandbox({
        id: "doc",
        name: "WillPrint"
  ...
  async #dispatchPageOpen(pageNumber, initialize = false) {
    ...
    if (initialize) {
      this.#closeCapability = createDeferred();
    }
...
class Autolinker {
  ...
  static findLinks(text) {
    this.#regex ??= /\b(?:https?:\/\/|mailto:|www\.)(?:[\S--[\p{P}<>]]|\/|[\S--[\[\]]]+[\S--[\p{P}<>]])+|\b[\S--[@\p{Ps}\p{Pe}<>]]+@([\S--[\p{P}<>]]+(?:\.[\S--[\p{P}<>]]+)+)/g;
...
class PDFViewer {
  ...
  async #onePageRenderedOrForceFetch(signal) {
    ...
    const hiddenCapability = createDeferred(),
  ...
  _resetView() {
    ...
    this._firstPageCapability = createDeferred();
    this._onePageRenderedCapability = createDeferred();
    this._pagesCapability = createDeferred();
