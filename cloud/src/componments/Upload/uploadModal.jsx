       ...
       fileReader.onload = e => {
         ...
         if (md5.length != 0) {
           ...
           if(nameSpace=='organization'){
              const orgIds=JSON.parse(window.localStorage.getItem('orgIds_cloud'))
              dispatch({
                ...
                payload: {
                  ...
                  fileEncryption: md5,
                  filePath: `${history.location.pathname.slice(1)}/${dataFormat(
                    String(new Date().getTime()).slice(0, 10),
                    'YYYY-MM-DD',
                  )}${typeName == '' ? '' : '/' + typeName}/${file.name}`,
                  orgCenterId: orgIds.orgCenterId,
                  buckName:orgIds.tenantId
                },
