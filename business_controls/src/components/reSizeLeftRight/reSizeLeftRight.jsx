...
const ReSizeLeftRight = (props) => {
  ...
  useEffect(() => {
    if (size?.width) {
      let vNumWidth =
        typeof props.vNum == 'string' && props.vNum.includes('%')
          ? toPoint(props.vNum) * size?.width
          : props.vNum;
      setVNum(vNumWidth);
      if (isExpandLeft) {
        dispatch({
  ...
  return (
    ...
    >
      ...
      >
        {!isShowLeft && isExpandLeft && (
          <b className={styles.drag_bt}>
            <img src={expandIcon} onClick={closeFn.bind(this)} />
          </b>
        )}
        {/* <b className={styles.drag_bt}><LeftOutlined /></b> */}
      </div>
      {isShowLeft && isExpandLeft && (
        <b className={styles.drag_expand}>
          <img src={expandIcon} onClick={openFn.bind(this)} />
        </b>
      )}
      <div
